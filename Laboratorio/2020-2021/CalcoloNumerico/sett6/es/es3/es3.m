...
[zero2,res3,iterates3,flag3]=NewtonMod(f3,df3,x0,3,toll,itmax,method);
[zero5,res5,iterates5,flag5]=NewtonMod(f5,df5,x0,5,toll,itmax,method);
...
hold on
semilogy(s3(2:end)./s3(1:end-1).^2,'g-o');
semilogy(s5(2:end)./s5(1:end-1).^2,'k-+');
hold off
legend('s_{k+1}/{s_k}^p m=1','s_{k+1}/{s_k}^p m=3',...
    's_{k+1}/{s_k}^p m=5',...
    'Location','South')
...
%% punto 5
figure(6)
semilogy(err_1(2:end)./s1.^2,'b-*'); 
hold on
semilogy(err_3(2:end)./s3.^2,'g-o');
semilogy(err_5(2:end)./s5.^2,'k-+');
