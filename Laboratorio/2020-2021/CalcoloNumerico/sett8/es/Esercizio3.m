...
for k = 1:M
    p_equi = polyfit(xinterp_equi,yinterp_equi(:,k),n);
    yeval_equi(:,k) = polyval(p_equi,xeval);
    p_cheb = polyfit(xinterp_cheb,yinterp_cheb(:,k),n);
    yeval_cheb(:,k) = polyval(p_cheb,xeval);
end
